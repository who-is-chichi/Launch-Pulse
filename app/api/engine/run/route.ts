...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!brand) {
      return NextResponse.json({ error: `Brand '${brandCode}' not found` }, { status: 404 });
    }
    ...
    try {
      ...
      logger.error('evaluateActions failed', { route: '[engine/run]', error: evalErr instanceof Error ? evalErr.message : String(evalErr) });
    }
    ...
    logger.error('Engine run failed', { route: '[engine/run]', error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
